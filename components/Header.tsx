    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-neutral-950/70 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/60">
      ...
      {open && (
        <div className="md:hidden">
          <nav
            id="mobile-nav"
            className="mx-4 mb-4 rounded-3xl border border-white/30 bg-neutral-900/90 p-4 shadow-xl backdrop-blur"
          >
            <div className="flex flex-col items-center gap-4">
              {nav.map(([label, href]) => (
                <a
                  key={href}
                  href={href}
                  className="w-full max-w-[560px] rounded-full border border-white/15 bg-white/5 px-5 py-3 text-center text-base font-medium text-white/90 hover:bg-white/10"
